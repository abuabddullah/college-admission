  ...
  try {
    ...
    const colleges = await College.insertMany([
      ...
      },
      {
        ...
        tuitionFee: 53000,
        gallery: ["/mit-campus.png", "/university-lab.jpg", "/university-library-interior.png"],
      },
      {
        ...
        tuitionFee: 54000,
        gallery: ["/harvard-campus.png", "/university-library-interior.png", "/university-sports-complex.jpg"],
      },
      {
      ...
      },
      {
        ...
        tuitionFee: 46000,
        gallery: ["/cambridge-campus.png", "/university-library-interior.png", "/university-lab.jpg"],
      },
      {
        name: "UC Berkeley",
        ...
        tuitionFee: 42000,
        gallery: ["/berkeley-campus.png", "/university-sports-complex.jpg", "/university-library-interior.png"],
      },
    ])
...
// Update user profile
app.put("/api/auth/profile", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
app.get("/api/colleges/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params
...
// Create new college
app.post("/api/colleges", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const {
      name,
      location,
    ...
    if (!name || !location || !description) {
      res.status(400).json({ error: "Name, location, and description are required" })
      return
    }
...
// Update college
app.put("/api/colleges/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    const updatedCollege = await College.findByIdAndUpdate(id, req.body, { new: true, runValidators: true })
...
// Delete college
app.delete("/api/colleges/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
...
app.get("/api/bookings", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const bookings = await Booking.find({ userId: req.userId }).populate("collegeId")
    res.json(bookings)
  } catch (error) {
...
// Get single booking
app.get("/api/bookings/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    const booking = await Booking.findOne({ _id: id, userId: req.userId }).populate("collegeId")
    if (!booking) {
...
// Create new booking
app.post("/api/bookings", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const {
      collegeId,
      studentName,
    ...
    if (!collegeId || !studentName || !email || !phone || !course || !previousEducation || !grade || !address) {
    ...
    const populatedBooking = await Booking.findById(newBooking._id).populate("collegeId")
...
app.put("/api/bookings/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    const updatedBooking = await Booking.findOneAndUpdate({ _id: id, userId: req.userId }, req.body, {
      new: true,
      runValidators: true,
    }).populate("collegeId")
...
// Delete booking
app.delete("/api/bookings/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    const deletedBooking = await Booking.findOneAndDelete({ _id: id, userId: req.userId })
    if (!deletedBooking) {
      res.status(404).json({ error: "Booking not found" })
      return
    }
...
app.get("/api/reviews/college/:collegeId", async (req: Request, res: Response): Promise<void> => {
  try {
...
app.get("/api/reviews/user", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const reviews = await Review.find({ userId: req.userId }).populate("collegeId").sort({ createdAt: -1 })
    res.json(reviews)
  } catch (error) {
...
// Create new review
app.post("/api/reviews", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { collegeId, rating, comment } = req.body

    if (!collegeId || !rating || !comment) {
      res.status(400).json({ error: "College ID, rating, and comment are required" })
      return
    }
    ...
    const existingReview = await Review.findOne({ userId: req.userId, collegeId })
    if (existingReview) {
      res.status(400).json({ error: "You have already reviewed this college" })
      return
    }
...
// Update review
app.put("/api/reviews/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    await College.findByIdAndUpdate(review.collegeId, { rating: Math.round(averageRating * 10) / 10 })
...
// Delete review
app.delete("/api/reviews/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    if (allReviews.length > 0) {
      const averageRating = allReviews.reduce((sum, r) => sum + r.rating, 0) / allReviews.length
      await College.findByIdAndUpdate(collegeId, { rating: Math.round(averageRating * 10) / 10 })
    } else {
...
app.get("/api/admin/bookings", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const bookings = await Booking.find().populate("userId collegeId").sort({ createdAt: -1 })
    res.json(bookings)
  } catch (error) {
...
app.put("/api/admin/bookings/:id", authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    const { status } = req.body
    ...
    const updatedBooking = await Booking.findByIdAndUpdate(id, { status }, { new: true }).populate("userId collegeId")
